<template>
  <v-card
    class="mb-2 card-item"
    variant="tonal"
    :ripple="false"
    @click="$emit('click', card)"
  >
    <div
      v-if="card.priority"
      class="priority-bar"
      :style="{ backgroundColor: priorityColorMap[card.priority] || 'transparent' }"
    />
    <v-card-text class="pa-3 pb-2">
      <div class="d-flex align-center ga-1">
        <v-icon v-if="card.linkedHabitId" size="14" icon="mdi-repeat" color="success" />
        <div class="text-body-2 font-weight-medium flex-grow-1 card-title">{{ card.title }}</div>
      </div>
      <div v-if="card.description" class="text-caption text-medium-emphasis mt-1 card-desc">
        {{ card.description }}
      </div>
      <div v-if="card.priority || card.estimatedTime || card.isFromInheritance" class="d-flex align-center mt-2 ga-1 flex-wrap">
        <v-chip v-if="card.priority" size="x-small" :color="priorityChipColor[card.priority]" variant="tonal">
          {{ priorityLabel[card.priority] || card.priority }}
        </v-chip>
        <v-chip v-if="card.estimatedTime" size="x-small" variant="outlined" prepend-icon="mdi-clock-outline">
          {{ formatTime(card.estimatedTime) }}
        </v-chip>
        <v-tooltip v-if="card.isFromInheritance" text="继承自前一天" location="top">
          <template #activator="{ props: tipProps }">
            <v-icon v-bind="tipProps" size="14" icon="mdi-arrow-left-bold" color="grey" />
          </template>
        </v-tooltip>
      </div>
    </v-card-text>
  </v-card>
</template>

<script setup lang="ts">
import type { Card } from '@kanban/shared'
import { Priority } from '@kanban/shared'

defineProps<{ card: Card }>()
defineEmits<{ click: [card: Card] }>()

const priorityColorMap: Record<string, string> = {
  [Priority.VH]: '#E8B4B8',
  [Priority.VN]: '#B4C5E8',
  [Priority.IH]: '#E8CEB4',
  [Priority.IN]: '#B4E8C0',
}

const priorityChipColor: Record<string, string> = {
  [Priority.VH]: 'red',
  [Priority.VN]: 'blue',
  [Priority.IH]: 'orange',
  [Priority.IN]: 'green',
}

const priorityLabel: Record<string, string> = {
  [Priority.VH]: '紧急重要',
  [Priority.VN]: '重要',
  [Priority.IH]: '紧急',
  [Priority.IN]: '一般',
}

const formatTime = (minutes: number) => {
  if (minutes < 60) return `${minutes}m`
  const h = Math.floor(minutes / 60)
  const m = minutes % 60
  return m > 0 ? `${h}h${m}m` : `${h}h`
}
</script>

<style scoped>
.card-item {
  cursor: grab;
  overflow: hidden;
  user-select: none;
  transition: transform 0.1s ease, box-shadow 0.15s ease;
}
.card-item:hover {
  transform: translateY(-1px);
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08);
}
.card-item:active { cursor: grabbing; transform: translateY(0); }
.priority-bar { height: 3px; width: 100%; }
.card-title { line-height: 1.4; }
.card-desc {
  display: -webkit-box;
  -webkit-line-clamp: 2;
  -webkit-box-orient: vertical;
  overflow: hidden;
  line-height: 1.4;
}
</style>
